...
allowed_fields = {
    ...
    "(AOP004)Concessions,patents,licenses and similar rights and other intangible assets":["Concessions,industrial rights,licences","Concessions, patents, licenses, trade marks etc.", "Service marks,software and other intangible assets","Concessions,patents,licenses and similar rights and other intangible assets"],
    ...
    "(AOP202)Revenues from sales": ["Revenues from sales","Income from sales","Income from sales (outside group)"],
        ...
        if "name" in obj and "value" in obj:
            ...
            for field, aliases in allowed_fields.items():
                if any(name.lower() == alias.lower() for alias in aliases):
                    ...
                    # Collect date if present
                    if "date" in obj:
                        extracted[field]["date"].append(obj["date"])

                    # Collect fromAmount if present
                    if "fromAmount" in obj:
                        extracted[field]["fromAmount"].append(obj["fromAmount"])
